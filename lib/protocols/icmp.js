...
module.exports = async (host, timeout, threads, packet) => {
    /*
     * -q: hide output
    ...
    */
    ...
    await pre_exec(`ping ${host} -q -p 0 -s ${packet} -w 5 -c 1`, (error, stdout, stderr) => {
        if (error) {
            console.log(error);
            console.log('\nstop all process.');
            process.exit(1);
        ...
        ICMP(host, timeout, threads, packet);
    });
}


const ICMP = async (host, timeout, threads, packet) => {
