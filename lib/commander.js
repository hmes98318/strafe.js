...
program.command('udp')
    ...
    .option('-c, --content [string]', 'Set the content of the sent data packet', [0x00])    // default 1 Bytes, maximum 1472 Bytes
    ...
    .action((options, command) => {
        ...
        confirm().then(() => {
            udp(command.args[0], options.port, options.broadcast, options.timeout, options.threads, options.content);
        })
            .catch((error) => {
                console.log(error);
                process.exit(cst.ERROR_EXIT);
...
program.command('icmp')
    ...
    .option('-t, --timeout [string]', 'Usage time(s)', '30')                                // default 30s
    .option('-T, --threads [string]', 'Number of Threads', '10')                            // default 10 Threads
    .option('-s, --packet [string]', 'Set packet payload size(bytes)', '56')                 // default 56 Bytes, maximum 1472 Bytes
    ...
    .action((options, command) => {
        ...
        console.log('timeout: ', options.timeout);
        console.log('threads: ', options.threads);
        console.log('packet: ', options.packet);


        confirm().then(() => {
            icmp(command.args[0], options.timeout, options.threads, options.packet);
        })
            .catch((error) => {
                console.log(error);
                process.exit(cst.ERROR_EXIT);
